# OmniPacker Windows ARM64 Build Script
...
Write-Host "========================================"
Write-Host "Building OmniPacker for Windows ARM64"
...
try {
    Write-Host "Modifying tauri.conf.json to only include win-arm64 resources..."
    $config = Get-Content $configFile -Raw | ConvertFrom-Json
    $config.bundle.resources = @("binaries/win-arm64/*")
    $config | ConvertTo-Json -Depth 10 | Set-Content $configFile

    Write-Host "Removing non-target platform binaries..."
    Get-ChildItem -Path $binariesDir -Directory | Where-Object { $_.Name -ne "win-arm64" } | ForEach-Object {
    ...
    Write-Host "  MSI: src-tauri\target\aarch64-pc-windows-msvc\release\bundle\msi\"
    Write-Host "  NSIS: src-tauri\target\aarch64-pc-windows-msvc\release\bundle\nsis\"
    Write-Host "========================================"
    ...
    $bundleDir = Join-Path $projectRoot "src-tauri\target\aarch64-pc-windows-msvc\release\bundle"
